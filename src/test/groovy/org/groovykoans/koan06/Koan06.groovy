...
class Koan06 extends GroovyTestCase {
    ...
    void test03_FileIteration() {
        ...
        int count = 0
        // ------------ START EDITING HERE ----------------------
        new File('src').eachFileRecurse(groovy.io.FileType.FILES, { File f ->
          if (f.text.contains('Lorem')) {
            count++
          }
        })
        // ------------ STOP EDITING HERE  ----------------------
    ...
    void test04_ConcludingExercise() {
        ...
        // ------------ START EDITING HERE ----------------------
        primesBetween200And250 = (200..250).grep{ candidate ->
          (2..<candidate).every{ candidate % it != 0 }
        }
        // ------------ STOP EDITING HERE  ----------------------
